...
namespace dit {
    ...
    public:
        ...
        delegate(delegate&& rhs)
            : object_ptr_(std::move(rhs.object_ptr_))
            , stub_ptr_(std::move(rhs.stub_ptr_))
            , deleter_(std::move(rhs.deleter_))
            , store_(std::move(rhs.store_))
            , store_size_(std::move(rhs.store_size_))
        {
            rhs.object_ptr_ = nullptr;
            rhs.stub_ptr_ = nullptr;
            rhs.store_.reset(nullptr);
            rhs.store_size_ = 0;
        }

        delegate(::std::nullptr_t const)  : delegate() {}
...
namespace std
{
    ...
    struct hash < dit::delegate<R(A...)> >
    {
        size_t operator()(dit::delegate<R(A...)> const& d) const 
        {
            auto const seed(hash<void*>()(d.object_ptr_));

            return hash<typename dit::delegate<R(A...)>::stub_ptr_type>()(
