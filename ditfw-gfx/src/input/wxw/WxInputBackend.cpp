...
namespace dit {
    ...
    void WxInputBackend::ProcessCharEvent(const wxKeyEvent& event)
    {
        KeyEvent ke = KeyboardEventType::KeyText;
        ke.rawCode = event.GetRawKeyCode();
        keyboard.ProcessEvent(ke);
    }
    ...
    void WxInputBackend::ProcessMouseEvent(const wxMouseEvent& event)
    {
#if 0
    ...
    static MouseEventType ConvertFromWxMouseType(const wxMouseEvent& event)
    {
        ...
        return isDown ? MouseEventType::ButtonPress : MouseEventType::ButtonRelease;
    }
    ...
    static Keys ConvertFromWxKey(int keyCode)
    {
        ...
        default:
            log("could not map wxWidgets keycode: %0", keyCode);
            return Keys::Escape;
        }
    }
