...
namespace glmesh
{
	...
	VertexFormat::VertexFormat(const AttributeList& attribs)
		: attributes(attribs)
	{
		m_attribOffsets.reserve(attributes.size());
		...
		for (const auto& attr : attributes)
		{
			...
			if (attr.GetVertexDataType() == VDT_DOUBLE_FLOAT)
				currOffset = ((currOffset + 7) / 8) * 8;

			m_attribOffsets.push_back(currOffset);
			currOffset += attr.ByteSize();

			//Round currOffset up to the nearest 4 bytes.
			currOffset = ((currOffset + 3) / 4) * 4;

			if (foundIndices.find(attr.GetAttribIndex()) != foundIndices.end())
				throw AttributeIndexMultipleRefException(attr.GetAttribIndex());

			foundIndices.insert(attr.GetAttribIndex());
		}
	...
	{
		BOOST_FOREACH(const AttribDesc &attribute, attributes)
		{
